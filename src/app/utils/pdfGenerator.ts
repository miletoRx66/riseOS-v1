...
import { departamentos, kpisDepartamento, tarefas, okrs } from "../data/mockData";

// Função auxiliar para adicionar header no PDF
const addPDFHeader = (doc: jsPDF, title: string) => {
  // Logo/Marca Rise
  doc.setFillColor(20, 233, 188); // #14E9BC
  ...
  return 45; // Retorna a posição Y onde o conteúdo deve começar
};
...
export const exportarRelatorioGeral = () => {
  ...
  const taxaConclusao = ((tarefasConcluidas / totalTarefas) * 100).toFixed(1);
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ["Total de Membros", `${totalMembros}`, `Média: ${(totalMembros / departamentos.length).toFixed(0)}/dept`],
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(20, 233, 188);
  doc.text("OKRs - Q1 2026", 20, yPosition);
  yPosition += 10;

  const okrData = okrs.map((okr) => {
    const dept = departamentos.find((d) => d.id === okr.departamento);
    return [dept?.nome || "", okr.titulo, `${okr.progresso}%`, okr.responsavel];
  });
  ...
  const insights = [
    "• Taxa de conclusão geral está 12% acima do período anterior, indicando melhora na produtividade.",
    "• Departamento de Produto apresenta o melhor desempenho com 91% de progresso em OKRs.",
    "• Marketing e Comercial estão alinhados com as metas de expansão e branding.",
    "• Operações está implementando processos de automação que devem impactar positivamente os próximos trimestres.",
    "• Recomenda-se manter o foco na execução de tarefas críticas para Q1 2026.",
  ];
...
export const exportarRelatorioDepartamento = (departamentoId: string) => {
  ...
  // Ícone/Badge do Departamento (simulado com cor)
  const corRGB = dept.cor.match(/\w\w/g)?.map((x) => parseInt(x, 16)) || [20, 233, 188];
  doc.setFillColor(corRGB[0], corRGB[1], corRGB[2], 0.2);
  ...
  autoTable(doc, {
    ...
    headStyles: {
      fillColor: [corRGB[0], corRGB[1], corRGB[2]],
      textColor: [0, 0, 0],
      fontStyle: "bold",
    },
    alternateRowStyles: {
  ...
  const okr = okrs.find((o) => o.departamento === dept.id);
  ...
  if (okr) {
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(20, 233, 188);
    doc.text("OKR Principal - Q1 2026", 20, yPosition);
    yPosition += 10;
    ...
    doc.setFont("helvetica", "normal");
    doc.setTextColor(189, 189, 189);
    doc.text(`Progresso Geral: ${okr.progresso}% | Responsável: ${okr.responsavel}`, 20, yPosition);
    yPosition += 10;

    const krData = okr.keyResults.map((kr) => [
      kr.descricao,
      `${kr.valorAtual} / ${kr.valorMeta} ${kr.unidade}`,
      `${kr.progresso}%`,
    ]);
  ...
  autoTable(doc, {
    ...
    body: [
      [
        "Planejamento",
        `${plannedTasks}`,
        `${deptTasks.length > 0 ? ((plannedTasks / deptTasks.length) * 100).toFixed(1) : 0}%`,
      ],
      [
        "Em Andamento",
        `${inProgressTasks}`,
        `${deptTasks.length > 0 ? ((inProgressTasks / deptTasks.length) * 100).toFixed(1) : 0}%`,
      ],
      [
        "Concluído",
        `${completedTasks}`,
        `${deptTasks.length > 0 ? ((completedTasks / deptTasks.length) * 100).toFixed(1) : 0}%`,
      ],
    ],
    theme: "grid",
    styles: {
  ...
  // Salvar PDF
  doc.save(
    `Relatorio_${dept.nome}_${new Date().toLocaleDateString("pt-BR").replace(/\//g, "-")}.pdf`
  );
};
