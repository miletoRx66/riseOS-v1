...
export default function Messages() {
  ...
  return (
    <div className="h-screen bg-rise-bg flex overflow-hidden">
      {/* Sidebar */}
      <div className="w-[300px] bg-rise-surface border-r border-rise-line flex flex-col flex-shrink-0">
        <div className="p-4 border-b border-rise-line">
          <div className="flex items-center justify-between mb-4">
            <h1 className="font-['Inter:Bold',sans-serif] text-rise-fg text-[22px]">Mural de Comunicação</h1>
          </div>
          {/* Tabs */}
          <div className="flex gap-1 bg-rise-raised rounded-lg p-1">
            {(["conversas", "canais"] as const).map((tab) => (
              <button
                ...
                className={`flex-1 py-1.5 rounded-md text-[13px] font-['Inter:Medium',sans-serif] transition-all ${
                  activeTab === tab ? "bg-[#14E9BC] text-[#000]" : "text-rise-fg-2 hover:text-rise-fg"
                }`}
              >
        ...
        <div className="flex-1 overflow-y-auto">
          {carregando ? (
            <div className="p-4 text-center text-rise-fg-4 text-[13px]">Carregando...</div>
          ) : activeTab === "conversas" ? (
            <div>
              <div className="px-4 py-3 flex items-center justify-between">
                <span className="text-rise-fg-4 text-[11px] uppercase tracking-wider font-['Inter:Medium',sans-serif]">
                  Mensagens Diretas
                </span>
                <button
              ...
              {mostrarNovaConversa && (
                <div className="px-3 mb-2">
                  <div className="relative">
                    <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-rise-fg-4" />
                    <input
                      autoFocus
                      type="text"
                      ...
                      className="w-full bg-rise-raised border border-rise-line rounded-lg pl-8 pr-3 py-2 text-rise-fg text-[13px] focus:border-[#14E9BC] focus:outline-none"
                    />
                  </div>
                  {buscaUsuario.length > 0 && (
                    <div className="mt-1 bg-rise-raised border border-rise-line rounded-lg overflow-hidden">
                      {usuariosFiltrados.slice(0, 5).map((u) => (
                        <button
                          key={u.id}
                          disabled={iniciandoConversa}
                          onClick={() => handleIniciarConversaP2P(u.id)}
                          className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-rise-raised transition-colors text-left"
                        >
                          <div className="w-8 h-8 rounded-full bg-[#14E9BC]/20 flex items-center justify-center text-[11px] font-bold text-[#14E9BC]">
                            {getIniciais(u.nome)}
                          </div>
                          <div>
                            <p className="text-rise-fg text-[13px] font-['Inter:Medium',sans-serif]">{u.nome}</p>
                            <p className="text-rise-fg-4 text-[11px]">{u.cargo ?? ""}</p>
                          </div>
                        </button>
                      ))}
                      {usuariosFiltrados.length === 0 && (
                        <p className="text-rise-fg-4 text-[12px] p-3 text-center">Nenhum usuário encontrado</p>
                      )}
                    </div>
                  )}
                </div>
              ...
              {canaisP2P.length === 0 ? (
                <p className="text-rise-fg-4 text-[13px] px-4 py-3">
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-3 transition-colors text-left ${
                        isAtivo ? "bg-[#14E9BC]/10 border-r-2 border-[#14E9BC]" : "hover:bg-rise-raised"
                      }`}
                    >
                      <div className="w-9 h-9 rounded-full bg-rise-raised flex items-center justify-center text-[12px] font-bold text-[#14E9BC] flex-shrink-0">
                        {outro ? getIniciais(outro.nome) : "?"}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-rise-fg text-[14px] font-['Inter:Medium',sans-serif] truncate">
                          {outro?.nome ?? "Conversa"}
                        </p>
                      </div>
                      <MessageSquare size={14} className="text-rise-fg-4 flex-shrink-0" />
                    </button>
                  );
                })
              )}
            ...
            <div>
              ...
              {canaisDept.length === 0 ? (
                <p className="text-rise-fg-4 text-[13px] px-4 py-3">Nenhum canal disponível.</p>
              ) : (
                canaisDept.map((c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Users size={14} className="text-rise-fg-4 flex-shrink-0" />
                    </button>
                  );
                })
              )}
      ...
      {canalAtivo ? (
        <div className="flex-1 flex flex-col min-w-0">
          {/* Header do chat */}
          <div className="h-[64px] border-b border-rise-line flex items-center px-6 gap-4 flex-shrink-0 bg-rise-surface">
            {canalAtivo.tipo === "p2p" ? (
              <>
                ...
                <div>
                  <p className="text-rise-fg text-[15px] font-['Inter:Semi_Bold',sans-serif]">{nomeCanalAtivo}</p>
                  <p className="text-rise-fg-4 text-[12px]">Mensagem direta</p>
                </div>
              </>
            ) : (
              <>
                ...
                <div>
                  <p className="text-rise-fg text-[15px] font-['Inter:Semi_Bold',sans-serif]">{nomeCanalAtivo}</p>
                  <p className="text-rise-fg-4 text-[12px]">Canal do departamento • {membrosCanal.length} membros</p>
                </div>
              </>
            )}
            <button
          ...
          <div className="flex flex-1 min-h-0">
            ...
            <div className="flex flex-col flex-1 min-w-0">
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4 space-y-1">
                {carregandoMsgs ? (
                  <div className="flex items-center justify-center h-full">
                    <p className="text-rise-fg-4 text-[14px]">Carregando mensagens...</p>
                  </div>
                ) : mensagens.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full gap-3">
                    ...
                    <p className="text-rise-fg-4 text-[14px]">Nenhuma mensagem ainda. Seja o primeiro!</p>
                  </div>
                ) : (
                  gruposMensagens.map(({ data, mensagens: msgs }) => (
                    <div key={data}>
                      <div className="flex items-center gap-3 my-4">
                        <div className="flex-1 h-px bg-rise-subtle" />
                        <span className="text-rise-fg-4 text-[11px] font-['Inter:Medium',sans-serif] whitespace-nowrap">
                          {data}
                        </span>
                        <div className="flex-1 h-px bg-rise-subtle" />
                      </div>
                      ...
                      {msgs.map((msg, i) => {
                        ...
                        return (
                          ...
                          >
                            {!sameSender ? (
                              <div className="w-8 h-8 rounded-full bg-rise-raised flex items-center justify-center text-[10px] font-bold text-[#14E9BC] flex-shrink-0 mb-0.5">
                            ...
                            <div className={`max-w-[65%] ${isOwn ? "items-end" : "items-start"} flex flex-col`}>
                              {!sameSender && (
                                <div className={`flex items-baseline gap-2 mb-1 ${isOwn ? "flex-row-reverse" : ""}`}>
                                  <span className="text-rise-fg text-[13px] font-['Inter:Semi_Bold',sans-serif]">
                                    {isOwn ? "Você" : nomeAutor}
                                  </span>
                                  <span className="text-rise-fg-4 text-[11px]">{formatarHora(msg.criado_em)}</span>
                                </div>
                              )}
                              <div
                                className={`px-4 py-2.5 rounded-2xl text-[14px] font-['Inter:Regular',sans-serif] leading-relaxed ${
                                  isOwn
                                    ...
                                    : "bg-rise-raised text-rise-fg rounded-bl-sm"
                                }`}
                              >
                              ...
                              {sameSender && (
                                <span className="text-rise-fg-4 text-[10px] mt-0.5 px-1">{formatarHora(msg.criado_em)}</span>
                              )}
                            </div>
                          </div>
                        );
              ...
              {/* Input de mensagem */}
              <div className="border-t border-rise-line p-4 bg-rise-surface flex-shrink-0">
                {/* Dropdown de @menção */}
                {showMentions && (
                  <div className="mb-2 bg-rise-raised border border-rise-line rounded-xl overflow-hidden shadow-lg">
                    <div className="px-3 py-1.5 border-b border-rise-line">
                      <span className="text-rise-fg-4 text-[11px] uppercase tracking-wider font-['Inter:Medium',sans-serif]">Mencionar</span>
                    </div>
                    {usuarios
                      ...
                      .map((u) => (
                        ...
                        >
                          ...
                          <div>
                            <p className="text-rise-fg text-[13px] font-['Inter:Medium',sans-serif]">{u.nome}</p>
                            {u.cargo && <p className="text-rise-fg-4 text-[11px]">{u.cargo}</p>}
                          </div>
                        </button>
                      ))}
                ...
                <div className="flex items-end gap-3">
                  <textarea
                    ...
                    rows={1}
                    className="flex-1 bg-rise-raised border border-rise-line rounded-xl px-4 py-3 text-rise-fg text-[14px] placeholder-[#555] focus:border-[#14E9BC] focus:outline-none resize-none font-['Inter:Regular',sans-serif] leading-relaxed"
                ...
                <p className="text-rise-fg-4 text-[11px] mt-2 font-['Inter:Regular',sans-serif]">
            ...
            {/* Painel de membros */}
            {mostrarMembros && (
              <div className="w-[220px] border-l border-rise-line bg-rise-surface flex flex-col flex-shrink-0">
                <div className="p-4 border-b border-rise-line flex items-center justify-between">
                  <span className="text-rise-fg text-[14px] font-['Inter:Semi_Bold',sans-serif]">
                    Membros ({membrosCanal.length})
                  </span>
                  <button
                    onClick={() => setMostrarMembros(false)}
                    className="text-rise-fg-4 hover:text-rise-fg transition-colors"
                  >
                    <X size={16} />
                  </button>
                </div>
                <div className="flex-1 overflow-y-auto p-2">
                  {membrosCanal.length === 0 ? (
                    <p className="text-rise-fg-4 text-[12px] p-3 text-center">Sem membros</p>
                  ) : (
                    membrosCanal.map((m) => (
                      <div key={m.id} className="flex items-center gap-2.5 p-2 rounded-lg hover:bg-rise-raised transition-colors">
                        ...
                        <div className="min-w-0">
                          <p className="text-rise-fg text-[13px] font-['Inter:Medium',sans-serif] truncate">{m.nome}</p>
                          {m.cargo && <p className="text-rise-fg-4 text-[11px] truncate">{m.cargo}</p>}
                        </div>
                      </div>
                    ))
                  )}
        ...
        <div className="flex-1 flex items-center justify-center flex-col gap-4">
          {carregando ? (
            ...
            <>
              <MessageSquare size={48} className="text-rise-fg-4" />
              <p className="text-rise-fg-4 text-[16px]">Selecione uma conversa ou canal</p>
            </>
          )}
        </div>
      )}
