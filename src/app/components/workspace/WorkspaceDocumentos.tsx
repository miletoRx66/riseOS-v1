  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative">
        <Search size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-rise-fg-3" />
        <input
          type="text"
          placeholder="Pesquisar documentos..."
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="w-full bg-rise-surface border border-rise-line rounded-lg pl-12 pr-4 py-3 text-rise-fg font-['Inter:Regular',sans-serif] text-[14px] focus:border-[#14E9BC] focus:outline-none"
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        {docsFiltrados.map((doc) => (
          <div
            key={doc.id}
            className="bg-rise-surface border border-rise-line rounded-lg p-4 hover:border-rise-fg-4 transition-all flex items-center justify-between"
          >
      ...
      {docsFiltrados.length === 0 && (
        <div className="bg-rise-surface border border-rise-line rounded-lg p-12 text-center">
          <FileText size={48} className="text-rise-fg-3 mx-auto mb-4" />
          <p className="font-['Inter:Medium',sans-serif] text-rise-fg-2 text-[16px] mb-2">
            {searchTerm ? "Nenhum documento encontrado" : "Nenhum documento disponível"}
          </p>
          <p className="font-['Inter:Regular',sans-serif] text-rise-fg-3 text-[14px]">
